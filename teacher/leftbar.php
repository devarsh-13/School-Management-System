 <div class="sidebar-menu">
            <div class="sidebar-header">
                <div class="logo">
                    <a href="dashboard.php"><img src="../images/header/ighs_sidebar.png" style="color: white;"  height= "80%" width="80%" alt="logo"  /></a>
                </div>
            </div>
            <div class="main-menu">
                <div class="menu-inner">
                    <nav>
                        <ul class="metismenu" id="menu">
                             <li >
                                <a href="dashboard.php" aria-expanded="true"><i class="ti-home"></i><span>Dashboard</span>
                            </a>
                        </li>
                            
                                             <li>
                                        <a href="view-profile.php"><i class="fa fa-user"></i> <span>Profile</span> </a>
                                     
                                    </li>
                                     
                                        
                            </li>
                          <li class="has-children">
                                       <a href="resource-class.php"><i class="fa fa-folder"></i> <span>Resources</span></a>    
                                    </li>

                                          <li class="has-children">
                                        <a href="#"><i class="fa fa-users"></i> <span>Chat</span> </i></a>
                                        <ul class="child-nav">
                                            <li><a href="../teacher_chat/select_students.php"><i class="fa fa-bars"></i> <span>Chat</span></a></li>
                                            <li><a href="manage_student_chat.php"><i class="fa fa fa-server"></i> <span>Manage Chat</span></a></li>
                                           
                                        </ul>
                                    </li>
                                     
                                    <li class="has-children">
                                      <a href="view-event.php"><i class="fa fa-file-text"></i> <span>Events</span></a>
                                    </li>

                                  

                                    <li class="has-children">
                                       <a href="view-gallery.php"><i class="fa fa-picture-o"></i> <span>Gallery</span></a>    
                                    </li>
                        </ul>
                    </nav>
                </div>
            </div>
        </div>